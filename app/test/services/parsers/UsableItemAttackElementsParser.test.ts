...
import { UsableItemAttackElementsParser } from "../../../src/services/parsers/UsableItemAttackElementsParser.ts";
...
describe("UsableItemAttackElementsParser", () =>
{
  ...
  {
    const note = "x\n<attackElements:[3, 1]>\n<attackElements:[1, 2]>";
    expect(UsableItemAttackElementsParser.readAttackElements(note)).toEqual([ 3, 1, 2 ]);
  });

  it("readAttackElements ignores invalid tokens", () =>
  {
    const note = "<attackElements:[1, bad, 2]>";
    expect(UsableItemAttackElementsParser.readAttackElements(note)).toEqual([ 1, 2 ]);
  });
  ...
  it("writeAttackElements replaces all prior lines and prepends one tag", () =>
  {
    ...
    expect(out).toContain("<attackElements:[4,5]>");
