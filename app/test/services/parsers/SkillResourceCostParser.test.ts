...
describe('SkillResourceCostParser granular reads', () =>
{
  it('readHpCostFlat pairs with writeHpCostFlat', () =>
  {
    ...
    expect(SkillResourceCostParser.readHpCostFlat(note))
      .toBe(11);
    expect(SkillResourceCostParser.readHpCostFlat(''))
      .toBe(0);
  });
  ...
  it('readHpCostFormula pairs with writeHpCostFormula', () =>
  {
    ...
    expect(SkillResourceCostParser.readHpCostFormula(note))
      .toBe('x');
  });
});
...
describe('SkillResourceCostParser granular writes', () =>
{
  it('writeHpCostFlat appends, replaces, and clears', () =>
  {
    ...
    expect(replaced)
      .toContain('<hp-cost:3>');
    expect(replaced.includes('<hp-cost:1>'))
      .toBe(false);
    expect(replaced)
      .toContain('keep');

    const cleared = SkillResourceCostParser.writeHpCostFlat('<hp-cost:9>', 0);
    expect(cleared.includes('hp-cost'))
      .toBe(false);
  });
...
describe('SkillResourceCostParser read/write integration', () =>
{
  it('round-trips HP flat, percent, formula, and can-kill', () =>
  {
    ...
    expect(SkillResourceCostParser.readHpCostFlat(note))
      .toBe(12);
    expect(SkillResourceCostParser.readHpCostPercent(note))
      .toBe(5);
    expect(SkillResourceCostParser.readHpCostFormula(note))
      .toBe('a.mhp * 0.02');
    expect(SkillResourceCostParser.readHpCostCanKill(note))
      .toBe(true);
    expect(note.includes('<custom:tag>'))
      .toBe(true);
  });
  ...
  it('round-trips MP and TP extra tag costs', () =>
  {
    ...
    expect(SkillResourceCostParser.readMpCostTagFlat(note))
      .toBe(4);
    expect(SkillResourceCostParser.readMpCostTagPercent(note))
      .toBe(10);
    expect(SkillResourceCostParser.readMpCostTagFormula(note))
      .toBe('a.mp');
    expect(SkillResourceCostParser.readTpCostTagFlat(note))
      .toBe(2);
    expect(SkillResourceCostParser.readTpCostTagPercent(note))
      .toBe(15);
    expect(SkillResourceCostParser.readTpCostTagFormula(note))
      .toBe('b.tp');
  });
