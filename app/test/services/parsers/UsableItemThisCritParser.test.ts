...
import { UsableItemThisCritParser } from "../../../src/services/parsers/UsableItemThisCritParser.ts";
...
describe("UsableItemThisCritParser", () =>
{
  it("readThisCritChance returns last matching line inner text", () =>
  {
    ...
    expect(UsableItemThisCritParser.readThisCritChance(note)).toBe("a.luk");
  });
  ...
  it("writeThisCritChance strips duplicates then prepends", () =>
  {
    ...
    expect(out.startsWith("<thisCritChance:[3]>")).toBe(true);
    expect(out.includes("<thisCritChance:[1]>")).toBe(false);
  ...
  it("writeThisCritDamageMultiplier clears when formula blank", () =>
  {
    ...
    expect(cleared.includes("thisCritDamageMultiplier")).toBe(false);
    expect(cleared).toContain("ok");
  });

  it("readThisCritsAlways and writeThisCritsAlways pair", () =>
  {
    const withTag = UsableItemThisCritParser.writeThisCritsAlways("", true);
    expect(UsableItemThisCritParser.readThisCritsAlways(withTag)).toBe(true);

    const cleared = UsableItemThisCritParser.writeThisCritsAlways(withTag, false);
    expect(UsableItemThisCritParser.readThisCritsAlways(cleared)).toBe(false);
  });
});
