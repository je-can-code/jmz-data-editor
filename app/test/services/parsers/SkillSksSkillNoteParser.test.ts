...
describe('SkillSksSkillNoteParser', () =>
{
  it('reads slot cost and explicit unslotted', () =>
  {
    expect(
      ...
      .toBe(3);
    expect(SkillSksSkillNoteParser.readExplicitUnslotted('<unslotted>'))
      .toBe(true);
    expect(SkillSksSkillNoteParser.readExplicitUnslotted('plain'))
      .toBe(false);
  });
  ...
  it('writeSksSkillTags replaces prior tags and prepends', () =>
  {
    ...
    expect(out)
      .toContain('<slotCost:2>');
    expect(out)
      .toContain('keep');
    expect(out.includes('9'))
      .toBe(false);
    expect(out.includes('unslotted'))
      .toBe(false);
  });
