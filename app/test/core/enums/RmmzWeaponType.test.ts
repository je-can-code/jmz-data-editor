...
describe('RmmzWeaponType', () =>
{
  ...
  {
    expect(normalizeRequiredWtypeId(0)).toBe(0);
    expect(normalizeRequiredWtypeId(2)).toBe(2);
    expect(normalizeRequiredWtypeId(2.7)).toBe(2);
    expect(normalizeRequiredWtypeId(-1)).toBe(0);
    expect(normalizeRequiredWtypeId(NaN)).toBe(0);
    expect(normalizeRequiredWtypeId(Infinity)).toBe(0);
  });
  ...
  it('weaponTypeOptionsFromNames falls back to None-only when empty', () =>
  {
    ...
    expect(o[ 0 ].group).toBe('No requirement');
  });
  ...
  it('weaponTypeOptionsFromNames maps indices and groups', () =>
  {
    ...
    expect(o[ 0 ].group).toBe('No requirement');
    expect(o[ 1 ].group).toBe('Weapon types');
    expect(o[ 2 ].group).toBe('Weapon types');
  });
  ...
  {
    expect(weaponTypeOptionForValue(1, [ 'None', 'Blade' ]).label).toBe('Blade');
    expect(weaponTypeOptionForValue(9, [ 'None', 'Blade' ]).value).toBe(9);
  });
});
